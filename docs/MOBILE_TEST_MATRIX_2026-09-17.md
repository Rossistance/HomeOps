...
| L6 | Onboarding | All pages advance; profile values round-trip unchanged; helpers page says none exist yet; Enter lands on Today | |
...
| T6 | Ask Famili card: suggestion text, mini composer, voice button, send | |
| T7 | Mini composer send opens Ask with the question fired | |
...
| A4 | Answer uses real household data (calendar/tasks) | |
...
| H9 | Autonomy "full" prompts for PIN; cancel leaves it unchanged | |
| H10 | Delete asks to confirm, then removes it from the list | |
| H11 | Save bar sits above the tab bar (fixed overlap) | |

## 4. Library (`(library)/index.tsx`, `upload-sheet.tsx`)

| ID | Check | Result |
|---|---|---|
| B1 | Files / Knowledge tabs with counts | |
...
| ID | Check | Result |
|---|---|---|
| C1 | Agenda view groups by day with week separators; past days absent | |
| C2 | Month view: seven columns, today highlighted, day tap selects and lists | |
| C3 | Previous/Next month | |
| C4 | Lens (Family / Nest / Me) filters | |
| C5 | Subscriptions row shows synced calendars; Sync runs | |
| C6 | New event opens the form | |
| C7 | Form: title, date/time, all-day, add/remove end time, notes, what to bring, remind row, Google toggle, Discard | |
| C8 | Save `TEST — delete me` event → appears in agenda and month | |
| C9 | Reopen event → edit → delete → gone; Google outcome shown honestly | |
