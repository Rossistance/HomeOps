...
export const familiUpdateTask = defineAction({
  ...
  async run(ctx, input) {
    ...
    // In the group thread a task the channel may not show is a missing one, word for word (see
    // famili.update_event). Elsewhere unchanged. (ADR-004.)
    if (!tk || tk.householdId !== hh || (channel === "group" && !seeable(tk))) return { ok: false, error: "task_not_found", message: "No such task — list tasks to find the right id." };
